...
static int io_read(void *opaque, uint8_t *buf, int buf_size) {
	...
	if (self->probe_buffer &&
		...
		size_t cnt =
		    MIN((size_t) buf_size, self->probe_buffer_size - self->probe_buffer_offs);
...
static void *ac_create_video_decoder(lp_ac_instance pacInstance,
	...
	const AVCodec *pCodec = avcodec_find_decoder(pFormatCtx->streams[nb]->codecpar->codec_id);
	AVCodecContext *pCodecCtx;
	...
	ERR(pDecoder->pCodec = (AVCodec*) avcodec_find_decoder(pDecoder->pCodecCtx->codec_id));
...
static void *ac_create_audio_decoder(lp_ac_instance pacInstance,
	...
	// Find correspondenting codec
	ERR(pDecoder->pCodec = (AVCodec*) avcodec_find_decoder(pCodecCtx->codec_id));
	...
	const struct AVChannelLayout layout = pCodecCtx->ch_layout;
	...
	if (av_sample_fmt_is_planar(fmt) || (fmt == AV_SAMPLE_FMT_S32) ||
		...
		ERR(swr_alloc_set_opts2(&pDecoder->pSwrCtx, &layout, out_fmt, rate, &layout, fmt, rate, 0, NULL));
...
static int ac_decode_audio_package(lp_ac_package pPackage,
	...
	if (pDecoder->pSwrCtx) {
		if (pDecoder->own_buffer_size != (size_t) buffer_size) {
			void *newbuffer;
...
EXTERN int CALL_CONVT ac_seek_frame(lp_ac_decoder pDecoder, int dir, int64_t frame) {
	AVFormatContext *ctx = ((lp_ac_data)pDecoder->pacInstance)->pFormatCtx;
	int flags = AVSEEK_FLAG_FRAME | (dir >= 0 ? 0 : AVSEEK_FLAG_BACKWARD);
	AV_ERR(av_seek_frame(ctx, pDecoder->stream_index, frame, flags));
	return 1;
error:
	return 0;
}
